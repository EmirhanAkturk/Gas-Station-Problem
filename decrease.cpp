/*
    ...
    durulması gereken istasyonlar problemi Decrease and conquer çözümü.
...
double cost(int c ,int idx)//idx:bulunulan istasyon,x:varılacak yer
{   //c: 1->oran,2->ucuz,3->uzak
    
    if((x-mList[idx])>f){//eger depodaki benzin kalan yolu gitmeye yetmeyecekse benzin al
        int y;//uğrayacağımız istasyon
        int isFirst=1;//ilk değer atama için kontrol 
        for(int j=idx+1;mList[j]-mList[idx]<=f;++j){//gidebildigi istasyona kadar hesapla
            double dist=mList[j]-mList[idx];
            if(dist<k)
                continue;
            ...
            if(isFirst){
                ...
                switch (c)
                {
                    case 1:{
                            ...
                            if(fp <= pList[y]/(mList[y]-mList[idx])){/*  (benzin fiyatı)/(mesafe)  oranı daha iyiyse bu durağa ugra*/
                            y=j;}
                            break;
                        }
        ...
        return pList[y]+cost(c,idx);//yolun kalanındaki maliyeti hesaplayıp toplama ekle
        }
    else{//kalan yolu gidecek kadar benzi varsa geriye doğru toplayarak döner
        return 0;
    }
}
...
double decrease(){
    double minCost=1000;//büyük bir değer ile ilklendirildi
    double c;
    
    c=cost(1,0);//oran olarak hesaplar
    if(c<minCost){//maliyet daha azsa 
        minCost=c;//min maliyeti güncelle
        minStations=stations;//min maliyet için durulan istasyonları güncelle
    }
    stations.clear();//sonraki kontrol için listeyi temizle

    c=cost(2,0);//ucuz istasyonlar için hesaplar
    if(c<minCost){//maliyet daha azsa 
        minCost=c;//min maliyeti güncelle
        minStations=stations;//min maliyet için durulan istasyonları güncelle
    }
    ...
    c=cost(3,0);//uzaktaki istasyonlar için hesaplar
    if(c<minCost){//maliyet daha azsa 
        minCost=c;//min maliyeti güncelle
        minStations=stations;//min maliyet için durulan istasyonları güncelle
    }
